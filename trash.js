...
// Если есть имя блока
if (blockName) {
  const dirPath = `${dir.trash}${blockName}/`; // полный путь к создаваемой папке блока
  mkdirp(dirPath, (err) => {                    // создаем
    // Если какая-то ошибка — покажем
    if (err) {
      console.error(`[IDM] Отмена операции: ${err}`);
    }

    // Нет ошибки, поехали!
    else {
      console.log(`[IDM] Создание папки: ${dirPath} (если отсутствует)`);
        ...
        if (extension === 'scss') {
          ...
          if (fileExist(imgFolder) === false) {
            mkdirp(imgFolder, (err) => {
              ...
              else console.log(`[IDM] Создание папки: ${imgFolder} (если отсутствует)`);
            });
          } else {
