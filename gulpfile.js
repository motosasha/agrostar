...
function compileTrash() {
	const fileList = [
		`${dir.trash}/trash.scss`,
	];
	return src(fileList, { sourcemaps: true })
		.pipe(plumber({
			errorHandler: function (err) {
...
exports.compileTrash = compileTrash;
...
function serve() {
	...
	// Стилевые глобальные файлы: все события (треш)
	watch([`${dir.trash}**/*.scss`], { events: ['all'], delay: 100 }, series(
		compileTrash,
		reload
	));
